...
- [Magi](https://magi-core-landing-2.vercel.app/) - Autonomous agent fleet driven by a shared board: a coordinator atomically claims each task for the least-loaded online worker (headless opencode core), and a supervisor reviews every done task, reopens subpar work, and turns each fault into a new task. Publishes its own weekly ops digest; live fleet dashboard at https://magi-fleet-status.vercel.app/.
