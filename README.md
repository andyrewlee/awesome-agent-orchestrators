...
- [OpenLLM](https://github.com/openllmsh/cli) - Local gateway daemon that puts every coding agent on the same provider chain: one alias walks Claude, ChatGPT, Kimi, Grok, Cursor subscriptions and API keys with automatic fallback when one hits its usage limit, plus shared code search and memory across Claude Code, Codex, OpenCode, Hermes, Pi, and any OpenAI-compatible client.
