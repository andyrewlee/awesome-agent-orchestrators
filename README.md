...
- [Unpeel](https://unpeel.com) - Native macOS app for running and supervising multiple CLI agent sessions (Claude Code, Codex, Gemini CLI, and more) with persistent terminals that survive app restarts, git worktree isolation, attention notifications, and iPhone remote control via a self-hosted E2E-encrypted relay. Closed-source, free download with a paid tier for remote access.
