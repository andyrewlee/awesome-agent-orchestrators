...
- [stapler](https://github.com/googlarz/stapler) - Self-hosted distribution of paperclip with zero-config onboarding (`npx @googlarz/stapler onboard`), embedded PostgreSQL (no external DB), trusted local loopback mode, and bind presets for LAN/Tailscale. Adds a first-class MCP server (`@stapler/mcp-server`) so Claude Code can manage the company — inbox, agents, issues, checkout — directly from a chat session.
