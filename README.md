...
- [tet](https://github.com/samil-kale/tet) - Lightweight agent IDE, simple by design: the real agent CLIs in split terminal tabs across several repositories, a git pane for what takes one or two clicks, and everything else left to an agent or a shell. Any project can run its agents in a Docker sandbox (sbx), and the agents' own configuration is never touched. Claude Code, Codex, OpenCode, Pi.
