...
- [mandate](https://github.com/culfin/mandate) - Each session gets a fixed role — developer, reviewer, maintainer — and its own git worktree; work comes from GitHub issues and moves on through pull requests, with no coordinator process running. Roles are permissions, not specialities. Bash and tmux. Claude Code; the Codex path ships unverified and says so.
