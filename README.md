...
- [kandev](https://github.com/kdlbs/kandev) - Self-hostable Kanban workbench for coding agents, with multi-step workflows that assign a different agent per step behind human gates. Parallel named sessions, git worktree isolation, multi-repository tasks, and an integrated editor, terminal, and diff review. Runs agents locally, in Docker, over SSH, or in cloud executors, and integrates with GitHub, GitLab, Azure DevOps, Jira, Linear, Sentry, and Slack. Multi-provider (Claude Code, Codex, Copilot, Gemini CLI, Amp, OpenCode, Cursor, and more) with no telemetry. ([kandev.ai](https://kandev.ai))
...
- [agentgit](https://github.com/Tryboy869/AgentGit) - Git-native multi-agent orchestration: agents work on isolated branches, and a deterministic validation script must exit 0 before an automated merge gate allows anything into main.
...
- [Claudexor](https://github.com/razzant/claudexor) - Local-first control plane that routes one coding thread across Claude Code, Codex, Cursor, and OpenCode, with quota-aware rotation across multiple Claude Code and Codex subscription profiles, Best-of-N runs, cross-family review, and typed apply gates.
