...
- [ouijit](https://github.com/ouijit/ouijit) - Desktop kanban for parallel CLI coding agent sessions in git worktrees. Agents inside Ouijit terminals get an auto-contextual `ouijit` CLI to update their own task state, set plans, and trigger hooks. Optional per-session Lima VM sandboxing. macOS and Linux.
