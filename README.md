...
- [orgh](https://github.com/reirei0126/orgh) - Watches an Obsidian inbox for mission notes; a Planner designs the task DAG, Claude Code and Codex workers run in parallel git worktrees, and a Reviewer gates each task — failed reviews resume the same session, plan-level defects escalate back to the Planner, and a Retro distills lessons into playbooks injected into later runs.
