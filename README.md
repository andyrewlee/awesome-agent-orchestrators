...
- [AgentPipe](https://github.com/Xiaofuziod/agentpipe) - Cross-vendor adversarial review pipeline: one agent writes (Claude), a different vendor reviews (Codex), looping until the reviewer is clean, with deterministic command gates (exit code = verdict) and fail-closed convergence. Rust engine + Tauri desktop GUI; macOS-first.
