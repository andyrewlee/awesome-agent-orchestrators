...
- [stapler](https://github.com/googlarz/stapler) - Self-hosted distribution of paperclip with zero-config onboarding (`npx @googlarz/stapler onboard`), embedded PostgreSQL, bind presets for loopback/LAN/Tailscale, Ollama adapter (full tool + memory parity with Claude adapter), company-scoped shared memories, COO agent (Chief Optimization Officer — process-level optimizer that runs outside the production pipeline), promptfoo-based agent evals framework, and a first-class MCP server (`@stapler/mcp-server`) so Claude Code can manage the company directly from a chat session.
