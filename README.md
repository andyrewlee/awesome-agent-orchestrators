...
- [RondoFlow](https://github.com/rondoflow/rondoflow) - Local-first visual canvas for orchestrating teams of Claude Code agents. Drag-and-drop React Flow board to compose agents, skills, MCP tools, and safety policies, then a Director steers the run mid-execution while a Planner and Advisor tune the team before and after. Multi-provider (Claude Code, OpenAI, Perplexity). Open-source, MIT.
