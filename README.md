...
- [pragma](https://github.com/akshaypimprikar/pragma) - Chains spec, plan, feature, gates, review, test, bugfix, and release for iOS/Xcode projects. `/gates` verifies 10 deterministic checks (build, full test suite, coverage, RED-before-GREEN commit order via git history, project-specific architecture rules) before a PR opens. `/loop` re-runs the next task and re-checks gates unattended until all pass.
